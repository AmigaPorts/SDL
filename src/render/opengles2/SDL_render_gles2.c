...
static GLES2_ProgramCacheEntry *GLES2_CacheProgram(GLES2_RenderData *data, GLuint vertex, GLuint fragment)
{
    ...
    entry->uniform_locations[GLES2_UNIFORM_PROJECTION] =
        data->myglGetUniformLocation(entry->id, "u_projection");
    entry->uniform_locations[GLES2_UNIFORM_TEXTURE_V] =
        data->myglGetUniformLocation(entry->id, "u_texture_v");
    entry->uniform_locations[GLES2_UNIFORM_TEXTURE_U] =
        data->myglGetUniformLocation(entry->id, "u_texture_u");
    entry->uniform_locations[GLES2_UNIFORM_TEXTURE] =
        data->myglGetUniformLocation(entry->id, "u_texture");
...
static int SetDrawState(GLES2_RenderData *data, const SDL_RenderCommand *cmd, const GLES2_ImageSource imgsrc, void *vertices)
{
    ...
    /* all drawing commands use this */
    {
        ...
        data->myglVertexAttribPointer(GLES2_ATTRIBUTE_COLOR, 4, GL_UNSIGNED_BYTE, GL_TRUE /* Normalized */, stride, (const GLvoid *)(base + offsetof(SDL_VertexSolid, color)));
    }
    ...
    while (cmd) {
        ...
        case SDL_RENDERCMD_CLEAR:
        {
            ...
            if (color != data->drawstate.clear_color) {
                ...
                data->myglClearColor(fr, fg, fb, fa);
