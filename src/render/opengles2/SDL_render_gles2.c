...
static bool GLES2_CreateTexture(SDL_Renderer *renderer, SDL_Texture *texture, SDL_PropertiesID create_props)
{
    ...
    if (data->yuv) {
        ...
        renderdata->myglTexImage2D(data->texture_type, 0, format, yuv_texture_w, yuv_texture_h, 0, format, type, NULL);
        if (!GL_CheckError("glTexImage2D()", renderer)) {
            SDL_free(data->pixel_data);
            SDL_free(data);
            return false;
    ...
    if (texture->format != SDL_PIXELFORMAT_EXTERNAL_OES) {
        ...
        if (!GL_CheckError("glTexImage2D()", renderer)) {
            if (!data->texture_external) {
                renderdata->myglDeleteTextures(1, &data->texture);
            }
            SDL_free(data->pixel_data);
            SDL_free(data);
            return false;
    ...
    if (texture->access == SDL_TEXTUREACCESS_TARGET) {
        renderdata->myglGenFramebuffers(1, &data->fbo);
        renderdata->myglBindFramebuffer(GL_FRAMEBUFFER, data->fbo);
        renderdata->myglFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, data->texture_type, data->texture, 0);
        const GLenum status = renderdata->myglCheckFramebufferStatus(GL_FRAMEBUFFER);
        renderdata->myglBindFramebuffer(GL_FRAMEBUFFER, renderer->target ? ((GLES2_TextureData *)renderer->target->internal)->fbo : renderdata->window_framebuffer);  // rebind previous fbo.
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            renderdata->myglDeleteFramebuffers(1, &data->fbo);
            if (!data->texture_external) {
                renderdata->myglDeleteTextures(1, &data->texture);
            }
            SDL_free(data->pixel_data);
...
static bool GLES2_SetRenderTarget(SDL_Renderer *renderer, SDL_Texture *texture)
{
    GLES2_RenderData *data = (GLES2_RenderData *)renderer->internal;
    data->drawstate.viewport_dirty = true;
    data->myglBindFramebuffer(GL_FRAMEBUFFER, texture ? ((GLES2_TextureData *)texture->internal)->fbo : data->window_framebuffer);
    return true;
}
...
static void GLES2_DestroyTexture(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (tdata) {
        if (tdata->fbo) {
            data->myglDeleteFramebuffers(1, &tdata->fbo);
        }
...
static bool GLES2_CreateRenderer(SDL_Renderer *renderer, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    if (SDL_GL_ExtensionSupported("GL_EXT_sRGB_write_control")) {
        data->myglDisable(GL_FRAMEBUFFER_SRGB);
    }
